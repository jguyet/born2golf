...
static void _golf_ui_aim_circle_name(golf_ui_layout_t *layout, const char *name, bool draw_aimer, float dt, vec2 *aim_delta, bool *start_aiming) {
    ...
    if (draw_aimer) {
        ...
        // vec2 aimer_size_top = V2(1 * ui.aim_circle.size, aimer_length);
        vec2 aimer_size_bottom = V2(1.5f * ui.aim_circle.size, aimer_length);
        ...
        if (aimer_length > max_length) {
            aimer_size_bottom.y = max_length / vert_scale; 
            aimer_size_bottom.y = max_length / vert_scale; 
            aimer_pos = vec2_add(pos, vec2_scale(delta, 0.5f * aimer_size_bottom.y));
        }
        game->aim_line.power = (aimer_length - min_length) / (max_length - min_length);

        vec4 wanted_color = V4(0, 0, 0, 1);
        float power = (aimer_size_bottom.y * vert_scale - min_length) / (max_length - min_length);
        ...
        float alpha = aimer_length / (max_length * 2.0f);
        if (alpha > 1) alpha = 1;
        circle_scale = 1 - alpha;

        vec_push(&ui.draw_entities, _golf_ui_draw_entity(texture->sg_image, aimer_pos, aimer_size_bottom, aimer_angle, V2(0, 0), V2(1, 1), 0, color, alpha)); 
